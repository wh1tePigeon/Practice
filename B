...
struct UInt {
    ...
    UInt(const std::vector<uint64_t>& digits);
    ...
    std::pair<UInt, UInt> div_mod(const UInt& other) const; // Целая часть и остаток от деления
                                                            ...
                                                            // Операторы:
    UInt& operator+=(const uint64_t num);     // Прибавление короткого
    ...
    UInt& operator*=(const uint64_t num);     // Умножение на короткое
    UInt& operator*=(const UInt& other); // Умножение на длинное
    UInt& operator/=(const uint64_t num);     // Деление на короткое
...
UInt operator-(const UInt&, const int);
UInt operator*(const UInt&, const uint64_t);
UInt operator*(const uint64_t, const UInt&);
UInt operator/(const UInt&, const uint64_t);
...
UInt::UInt(const std::vector<uint64_t>& digits) : digits(digits) {
    normalize();
}
...
UInt& UInt::operator+=(const uint64_t num) {
    assert(num >= 0);
    if (num >= BASE) {
...
UInt& UInt::operator+=(const UInt& other) {
    ...
    const uint64_t s1 = this->digits.size();
    const uint64_t s2 = other.digits.size();
    uint64_t rem = 0;
    for (int i = 0; i < s1 || i < s2 || rem > 0; ++i) {
        uint64_t d1 = i < s1 ? this->digits[i] : (digits.push_back(0), 0);
        uint64_t d2 = i < s2 ? other.digits[i] : 0;
        rem += d1 + d2;
        auto div = rem / BASE;
...
UInt UInt::slow_mult(const UInt& other) const {
    ...
    std::vector<uint64_t> temp(s1 + s2);
    for (int i = 0; i < s1; ++i) {
        uint64_t rem = 0;
        for (int j = 0; j < s2; ++j) {
...
uint64_t operator%(const UInt& a, const uint64_t num) {
    assert(num > 0);
    uint64_t rem = 0;
...
UInt operator-(const UInt& a, const int b) { return UInt(a) -= b; }
UInt operator*(const UInt& a, const uint64_t b) { return UInt(a) *= b; }
UInt operator*(const uint64_t a, const UInt& b) { return b * a; }
UInt operator/(const UInt& a, const uint64_t b) { return UInt(a) /= b; }
//UInt operator^(const UInt& a, const int n) { return pow(a, n); } // Возведение в степень

// Возведение в степень:
UInt pow(UInt a, int n, int64_t p) {
    UInt res = 1;
    while (n > 0) {
        if (n % 2 != 0) res *= a;
        res %= p;
...
int main() {
    ...
    cin >> p >> k;
    vector<uint64_t> inp;
    uint64_t gb = 0, message = 0;
    while (cin >> gb) {
        cin >> message;
        uint64_t key = pow(gb, k, p);
        uint64_t reverseElem = pow(key, p - 2, p);
        inp.push_back((reverseElem * message) % p);
    }
    ...
    for (int i = 1; i < inp.size(); ++i) {
        decimalSystem += inp[i] * temp;
        temp *= p;
    }

    vector<uint64_t> out;
    do {
        out.push_back(decimalSystem % 64);
        decimalSystem /= 64;
    } while (decimalSystem > 0);


    for (auto sym : out) {
        std::cout << number_to_char(sym);
    }
    std::cout << std::endl;
    return 0;
}
