...
int main() {
    unsigned long long p = 0, g = 0, k = 0;
    cin >> p >> g >> k;
    string message;
    getline(cin, message);
    getline(cin, message);
    ...
    for (auto digit : out) {
        int b = 1 + rand() % (p - 1);
        UInt temp(g);
        std::cout << ::pow(temp, b, p) % p << " ";
        temp = k;
        UInt temp2(pow(temp, b, p));
        temp2 *= digit;
        temp2 %= p;
